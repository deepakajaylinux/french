...
Ruby Ruby Version Manager (RVM), souvent abrégé en RVM, est une plate-forme logicielle de type Unix conçu pour gérer plusieurs installations de Ruby sur le même appareil. L'environnement rubis entier, y compris l'interpréteur Ruby, RubyGems installés, et la documentation est partitionné. Celles-ci comprennent JRuby, mruby, MacRuby, IronRuby, Maglev, Rubinius, Topaz. Il est amical avec Ubuntu et cent OS utilisateur. .

Commande Aide
--------------------

Cette commande peut fonctionner sur les objectifs et les commandes disponibles sous module Ruby RVM. Il explique également la commande de module Ruby RVM. Avant l'installation vous lisez cette commande d'aide explique sa fonction.
...
Ce est un processus flagrante à installer le module RubyRVM sous ptconfigure en utilisant simplement la commande ci-dessous,
...
.. cssclass:: table-bordered
 ...
 +==============================+==============================================+==============+======================================+
 |ptconfigure RubyRVM           | Nous pouvons utiliser RubyRVM, rubyrvm,      | Y            | Le système démarre processus         |
 |Install?(Y/N)                 | ruby-rvm, rubyRvm.                           |              | d'installation                       |
 +------------------------------+----------------------------------------------+--------------+--------------------------------------+
 |ptconfigure RubyRVM           | Nous pouvons utiliser RubyRVM, rubyrvm,      | N            | Système arrête processus             |
 |Install?(Y/N)                 | ruby-rvm, rubyRvm.                           |              | d'installation|                      |
 +------------------------------+----------------------------------------------+--------------+--------------------------------------+
...
* RVM est utile même si vous ne voulez pas installer plusieurs versions en même temps.
* Obtention à partir du gestionnaire de paquets (au moins dans ubuntu) vous donne une version mise à jour qui change répertoires d'installation   du défaut.
* Compilation des sources nécessite d'avoir une charge de dépendances et de la configuration des options.
* RVM est grande car cela permet à votre installer différentes versions sans toucher de votre système Ruby l'installation par défaut.
* Un autre grand avantage d'avoir RVM sont les gemsets - vous pouvez créer autant de gemsets qui sont uniques à la version et niveau de patch, 
  de ruby.
* Système Ruby est un, de réflexion, de haut niveau langage de script dynamique pour la programmation orientée objet rapide et facile (POO).
* Dans le système Ruby, il ya beaucoup outil pratique et agréable. Y compris les outils qui sont utilisés dans le processus de déploiement.
* Le système Ruby moyenne est habituellement déjà un professionnel ayant les connaissances extrême expérimenté à la fois dans le développement 
  web et générale compétences en génie logiciel.
...
* Des projets de mise en cache - l'une des étapes les plus importantes dans l'élaboration d'un projet majeur de l'Internet. En PHP, il existe 
  diverses options pour les données la mise en cache.
